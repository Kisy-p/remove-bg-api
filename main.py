...
async def startup_event():
    logger.info("Préchargement du modèle U2Net...")
    try:
        # Import et préchargement du modèle
        from rembg.bg import get_model
        start_time = time.time()
        model = get_model("u2net")
        logger.info(f"Modèle U2Net chargé en {time.time() - start_time:.2f} secondes")
    except Exception as e:
        logger.error(f"Erreur lors du chargement du modèle: {str(e)}")
        # Continuer malgré l'erreur, le modèle sera chargé à la première requête
